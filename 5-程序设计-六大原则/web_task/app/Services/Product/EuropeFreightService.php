...
class EuropeFreightService
{
    ...
    const EUROPE_COUNTRY_ID = [81, 222];
     ...
     * @return array
     */
    public function verifyProductRule($data)
    {
        $ret = [];
    ...
    public function getIdealProductFreight($value, $data, $country_id, $extra_fee = 0, $combo_child_info = null)
    {
        ...
        switch (count($value['to_info'])) {
            ...
            case 1:// 有唯一的to国别的报价
                ...
                $freight[$value['to_info'][0]->id] = self::calcFreight($data['rule'][$country_id]->extra_charge_ratio,
            ...
            default:
                ...
                foreach ($value['to_info'] as $ks => $vs) {
                    ...
                    $freight[$vs->id] = self::calcFreight($data['rule'][$country_id]->extra_charge_ratio,
                        $data['rule'][$country_id]->vat_ratio,
                        $vs->clearance_fee,
                        $freight_fee + $extra_fee,
                        $value['info']->freight
     ...
     * @return false|float
     */
    public static function calcFreight($coefficient_a, $coefficient_b, $clearance_fee, $freight_fee, $origin_fee)
    {
    ...
    {
        //保留三位小数，然后向上进一位
        $ret = sprintf('%.2f', ceil($freight * 100) / 100);
        if ($ret < 0) {
            $ret = '0.00';
        }
        return $ret;
    ...
    public function getBaseProductInfo($data)
    {
        ...
        // combo的话进行子sku的验证
        //获取验证规则
        $rule = DB::table('tb_sys_international_order_config')
            ->whereIn('country_id', self::EUROPE_COUNTRY_ID)
            ->where('status', 1)
            ->get()
            ->keyBy('country_id')
            ->toArray();
        $extra_rule = DB::table('tb_sys_international_order_extra_config')
            ->whereIn('country_id', self::EUROPE_COUNTRY_ID)
            ->get()
            ->toArray();
        $ret['rule'] = $rule;
        ...
        foreach ($data as $key => $value) {
            if (in_array($value['from'], [self::BRITAIN, self::GERMANY])) {
                ...
                if (isset($country_code_info[$value['from'] . '-' . $value['to']])) {
                    ...
                    // 根据邮编和to来确认是哪一个区域
                    $to_id = $this->getCountryIdByZipCode($this->get_need_string($value['zip_code'], [' ', '-', '*', '_']), $country_id, $value['to']);
                    $to_info = DB::table('tb_sys_international_order')
                        ->where([
                ...
                $temp = $value;
                if (isset($info[$value['product_id']])) {
     ...
     */
    public function get_need_string($string, $remove = [' '])
    {
    ...
    public function getGBRProductRule($value, $data)
    {
        ...
        if ($value['combo_info']) {
            ...
            if ($flag) {
                ...
                if ($combo_flag) {
                    $ret = $this->returnProduct(self::CODE_SUCCESS, $value['product_id'], $value, '', min($all_freight));
                }
            }
        } else {
            // 三个数字排序
    ...
    {
        return DB::table('tb_sys_international_order_postcode as p')
