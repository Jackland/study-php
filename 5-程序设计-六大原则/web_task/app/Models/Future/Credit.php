...
{
    protected  $table = 'oc_seller_credit';
    static $page_size = 15;

    public static function getCredit($seller_id)
    {
        return \DB::table('oc_seller_credit')
    ...
    public static function getBillList($seller_id, $page, $filter, $page_size = 15)
    {
        $build = \DB::table('oc_seller_credit_bill')
            ->where(['seller_id' => $seller_id])
            ->when(isset($filter['serial_no']) && $filter['serial_no'], function ($query) use ($filter) {
                return $query->where('serial_no', 'like', '%' . $filter['serial_no'] . '%');
            })
     ...
     * @return bool
     */
    public static function insertCreditBill($seller_id, $amount, $type = 1)
    {
