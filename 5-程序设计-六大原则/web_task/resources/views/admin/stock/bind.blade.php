...
@section('content')
    ...
    {!! Form::open(['url' => route('stock.bind'),'class'=>"form-inline",'method'=>'get']) !!}
    @component('component.form_group')
        {!! Form::input('text','sales_order_id',old('sales_order_id'),['class'=>"form-control",'placeholder'=>'Sales Oder ID']) !!}
    @endcomponent
    @component('component.form_group')
    ...
    @if (!empty($associatedList))
        ...
        <table class="table table-bordered">
            ...
            @foreach ($associatedList as $associated)
                <tr>
                    <th scope="row">{{ $loop->iteration }}</th>
                    <td>{{ $associated->sales_order_id }}</td>
                    <td>{{ $associated->sales_order_line_id }}</td>
