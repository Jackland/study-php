...
class ControllerReportStatistics extends Controller {
	...
	public function ordersale() {
		...
		if ($this->validate()) {
			...
			$this->response->redirect($this->url->link('report/statistics', 'user_token=' . session('user_token'), true));
		}
	...
	public function getList() {
		...
		foreach ($results as $result) {
			$data['statistics'][] = array(
				...
				'value' => $result['value'],
				'href'  => $this->url->link('report/statistics/' . str_replace('_', '', $result['code']), 'user_token=' . session('user_token'), true)
			);
		}
