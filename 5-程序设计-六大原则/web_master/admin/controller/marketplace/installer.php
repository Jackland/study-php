...
class ControllerMarketplaceInstaller extends Controller {
	...
	public function history() {
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('marketplace/installer/history', 'user_token=' . session('user_token') . '&page={page}', true);
	...
	public function upload() {
		...
		if (!$json) {
			...
			if (is_file($file)) {
				...
				$json['next'] = str_replace('&amp;', '&', $this->url->link('marketplace/install/install', 'user_token=' . session('user_token') . '&extension_install_id=' . $extension_install_id, true));		
			} else {
