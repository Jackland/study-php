...
class ControllerMarketplaceInstall extends Controller {
	public function install() {
		...
		} elseif (!is_file(DIR_UPLOAD . session('install') . '.tmp')) {
			$json['error'] = $this->language->get('error_file');
		}
		...
		if (!$json) {
			...
			$json['next'] = str_replace('&amp;', '&', $this->url->link('marketplace/install/unzip', 'user_token=' . session('user_token') . '&extension_install_id=' . $extension_install_id, true));
		}
	...
	public function unzip() {
		...
		if (!$json) {
			...
			if ($zip->open($file)) {
				$zip->extractTo(DIR_UPLOAD . 'tmp-' . session('install'));
				$zip->close();
			} else {
			...
			$json['next'] = str_replace('&amp;', '&', $this->url->link('marketplace/install/move', 'user_token=' . session('user_token') . '&extension_install_id=' . $extension_install_id, true));
		}
	...
	public function move() {
		...
		} elseif (!is_dir(DIR_UPLOAD . 'tmp-' . session('install') . '/')) {
			$json['error'] = $this->language->get('error_directory');
		}

		if (!$json) {
			$directory = DIR_UPLOAD . 'tmp-' . session('install') . '/';
		...
		if (!$json) {
			...
			$json['next'] = str_replace('&amp;', '&', $this->url->link('marketplace/install/xml', 'user_token=' . session('user_token') . '&extension_install_id=' . $extension_install_id, true));
		}
	...
	public function xml() {
		...
		if (!$json) {
			$file = DIR_UPLOAD . 'tmp-' . session('install') . '/install.xml';
		...
		if (!$json) {
			...
			$json['next'] = str_replace('&amp;', '&', $this->url->link('marketplace/install/remove', 'user_token=' . session('user_token'), true));
		}
