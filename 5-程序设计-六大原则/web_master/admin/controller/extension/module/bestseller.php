	...
	public function index() {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/bestseller', 'user_token=' . session('user_token'), true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/bestseller', 'user_token=' . session('user_token') . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/bestseller', 'user_token=' . session('user_token'), true);
		} else {
			$data['action'] = $this->url->link('extension/module/bestseller', 'user_token=' . session('user_token') . '&module_id=' . $this->request->get['module_id'], true);
		}

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=module', true);

		if (isset($this->request->get['module_id']) && (!request()->isMethod('POST'))) {
