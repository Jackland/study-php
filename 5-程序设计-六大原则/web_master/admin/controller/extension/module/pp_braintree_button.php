...
class ControllerExtensionModulePPBraintreeButton extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/pp_braintree_button', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/module/pp_braintree_button', 'user_token=' . session('user_token'), true);
		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=module', true);
		$data['layouts'] = $this->url->link('design/layout', 'user_token=' . session('user_token'), true);
	...
	public function configure() {
		...
		if (!$this->user->hasPermission('modify', 'extension/extension/module')) {
			...
			$this->response->redirect($this->url->link('design/layout', 'user_token=' . session('user_token'), true));
		}
	}
