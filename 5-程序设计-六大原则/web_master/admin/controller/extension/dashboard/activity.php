...
/**
 * @property ModelExtensionDashboardActivity $model_extension_dashboard_activity
...
class ControllerExtensionDashboardActivity extends Controller {
	...
	public function index() {
		...
		if ((request()->isMethod('POST')) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=dashboard', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=dashboard', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/dashboard/activity', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/dashboard/activity', 'user_token=' . session('user_token'), true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=dashboard', true);
	...
	public function dashboard() {
		...
		foreach ($results as $result) {
			...
			$replace = array(
				...
				$this->url->link('sale/return/edit', 'user_token=' . session('user_token') . '&return_id=', true)
			);
