...
/**
 * @property ModelExtensionPaymentG2aPay $model_extension_payment_g2apay
...
class ControllerExtensionPaymentG2APay extends Controller {
	...
	public function index() {
		...
		if ((request()->isMethod('POST')) && $this->validate()) {
			...
			session()->set('complete', $this->language->get('text_complete'));
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/g2apay', 'user_token=' . session('user_token'), true)
		);

		$this->load->model('localisation/order_status');
		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();

		$data['action'] = $this->url->link('extension/payment/g2apay', 'user_token=' . session('user_token'), true);
