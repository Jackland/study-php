...
/**
 * @property ModelExtensionPaymentSquareup $model_extension_payment_squareup
...
class ControllerExtensionPaymentSquareup extends Controller {
    ...
    public function index() {
        ...
        if ($previous_config->get('payment_squareup_access_token') && $previous_config->get('payment_squareup_access_token_expires')) {
            ...
            if ($delta < 0) {
                $this->pushAlert(array(
                    'type' => 'danger',
                    'icon' => 'exclamation-circle',
                    'text' => sprintf($this->language->get('text_token_expired'), $this->url->link('extension/payment/squareup/refresh_token', 'user_token=' . session('user_token'), true))
                ));
            } else if ($delta < (5 * 24 * 60 * 60)) { // token is valid, just about to expire
                $this->pushAlert(array(
                    'type' => 'warning',
                    'icon' => 'exclamation-circle',
                    'text' => sprintf($this->language->get('text_token_expiry_warning'), $expiration_date_formatted, $this->url->link('extension/payment/squareup/refresh_token', 'user_token=' . session('user_token'), true))
                ));
            }
        ...
        $data['payment_squareup_refresh_link'] = $this->url->link('extension/payment/squareup/refresh_token', 'user_token=' . session('user_token'), true);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/squareup', 'user_token=' . session('user_token'), true)
        );

        $data['action'] = html_entity_decode($this->url->link('extension/payment/squareup', 'user_token=' . session('user_token'), true));
        $data['action_save_auth'] = html_entity_decode($this->url->link('extension/payment/squareup', 'user_token=' . session('user_token') . '&save_and_auth=1', true));
        $data['cancel'] = html_entity_decode($this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=payment', true));
        $data['url_list_transactions'] = html_entity_decode($this->url->link('extension/payment/squareup/transactions', 'user_token=' . session('user_token') . '&page={PAGE}', true));
        ...
        $data['catalog'] = request()->serverBag->get('HTTPS') ? HTTPS_CATALOG : HTTP_CATALOG;
        ...
        if ($api_info && $this->user->hasPermission('modify', 'sale/order')) {
            ...
            $this->model_user_api->addApiSession($api_info['api_id'], $session->getId(), request()->serverBag->get('REMOTE_ADDR'));
    ...
    public function transaction_info() {
        ...
        if (empty($transaction_info)) {
            $this->response->redirect($this->url->link('extension/payment/squareup', 'user_token=' . session('user_token'), true));
        }
        ...
        $data['cancel'] = $this->url->link('extension/payment/squareup', 'user_token=' . session('user_token') . '&tab=tab-transaction', true);

        $data['url_order'] = $this->url->link('sale/order/info', 'user_token=' . session('user_token') . '&order_id=' . $transaction_info['order_id'], true);
        $data['url_void'] = $this->url->link('extension/payment/squareup' . '/void', 'user_token=' . session('user_token') . '&preserve_alert=true&squareup_transaction_id=' . $transaction_info['squareup_transaction_id'], true);
        $data['url_capture'] = $this->url->link('extension/payment/squareup' . '/capture', 'user_token=' . session('user_token') . '&preserve_alert=true&squareup_transaction_id=' . $transaction_info['squareup_transaction_id'], true);
        $data['url_refund'] = $this->url->link('extension/payment/squareup' . '/refund', 'user_token=' . session('user_token') . '&preserve_alert=true&squareup_transaction_id=' . $transaction_info['squareup_transaction_id'], true);
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/squareup/transaction_info', 'user_token=' . session('user_token') . '&squareup_transaction_id=' . $squareup_transaction_id, true)
        );
    ...
    public function transactions() {
        ...
        foreach ($transactions as $transaction) {
            ...
            $result['transactions'][] = array(
                ...
                'url_order' => $this->url->link('sale/order/info', 'user_token=' . session('user_token') . '&order_id=' . $transaction['order_id'], true),
                'url_void' => $this->url->link('extension/payment/squareup/void', 'user_token=' . session('user_token') . '&squareup_transaction_id=' . $transaction['squareup_transaction_id'], true),
                'url_capture' => $this->url->link('extension/payment/squareup/capture', 'user_token=' . session('user_token') . '&squareup_transaction_id=' . $transaction['squareup_transaction_id'], true),
                'url_refund' => $this->url->link('extension/payment/squareup/refund', 'user_token=' . session('user_token') . '&squareup_transaction_id=' . $transaction['squareup_transaction_id'], true),
                ...
                'url_info' => $this->url->link('extension/payment/squareup/transaction_info', 'user_token=' . session('user_token') . '&squareup_transaction_id=' . $transaction['squareup_transaction_id'], true)
            );
        }
    ...
    public function oauth_callback() {
        ...
        try {
            ...
            $this->session->remove('payment_squareup_oauth_state');
            $this->session->remove('payment_squareup_oauth_redirect');
    ...
    public function order() {
        ...
        $data['url_list_transactions'] = html_entity_decode($this->url->link('extension/payment/squareup/transactions', 'user_token=' . session('user_token') . '&order_id=' . $this->request->get['order_id'] . '&page={PAGE}', true));
    ...
    public function recurringButtons() {
        ...
        $data['cancel'] = html_entity_decode($this->url->link('extension/payment/squareup/recurringCancel', 'order_recurring_id=' . $order_recurring_id . '&user_token=' . session('user_token'), true));
    ...
    protected function clearAlerts() {
        $this->session->remove('payment_squareup_alerts');
    }

    protected function getSettingValue($key, $default = null, $checkbox = false) {
        if ($checkbox) {
            if (request()->isMethod('POST') && !isset($this->request->post[$key])) {
                return $default;
            } else {
