...
/**
 * @property ModelExtensionPaymentPilibaba $model_extension_payment_pilibaba
...
class ControllerExtensionPaymentPilibaba extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/pilibaba', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/payment/pilibaba', 'user_token=' . session('user_token'), true);
		...
		if ($this->config->get('config_weight_class_id') != '2') {
			$data['error_weight'] = sprintf($this->language->get('error_weight'), $this->url->link('setting/setting', 'user_token=' . session('user_token'), true));
		} else {
			$data['error_weight'] = '';
		}
			...
			if (utf8_strlen($this->request->post['email_address']) < 1) {
				...
				if (isset($response['code']) && isset($response['message'])) {
					if ($response['code'] == '0') {
						...
						session()->set('success', $this->language->get('text_register_success'));

						$json['redirect'] = $this->url->link('extension/payment/pilibaba', 'user_token=' . session('user_token'), true);
					} else {
	...
	public function order() {
		if ($this->config->get('payment_pilibaba_status')) {
			...
			if ($pilibaba_order) {
				...
				$data['barcode'] = $this->url->link('extension/payment/pilibaba/barcode', 'user_token=' . session('user_token') . '&order_id=' . $this->request->get['order_id'], true);
