...
/**
 * @property ModelExtensionPaymentFirstdata $model_extension_payment_firstdata
...
class ControllerExtensionPaymentFirstdata extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/firstdata', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/payment/firstdata', 'user_token=' . session('user_token'), true);
	...
	public function order() {
		if ($this->config->get('payment_firstdata_status')) {
			...
			if (!empty($firstdata_order)) {
				...
				$data['void_url'] = $this->url->link('extension/payment/firstdata/void', 'user_token=' . session('user_token'), true);
				$data['capture_url'] = $this->url->link('extension/payment/firstdata/capture', 'user_token=' . session('user_token'), true);
				$data['notify_url'] = HTTPS_CATALOG . 'index.php?route=extension/payment/firstdata/notify';
				...
				if (isset($this->session->data['void_success'])) {
					$data['void_success'] = session('void_success');

					$this->session->remove('void_success');
				} else {
					$data['void_success'] = '';
				}

				if (isset($this->session->data['void_error'])) {
					$data['void_error'] = session('void_error');

					$this->session->remove('void_error');
				} else {
					$data['void_error'] = '';
				}

				if (isset($this->session->data['capture_success'])) {
					$data['capture_success'] = session('capture_success');

					$this->session->remove('capture_success');
				} else {
					$data['capture_success'] = '';
				}

				if (isset($this->session->data['capture_error'])) {
					$data['capture_error'] = session('capture_error');

					$this->session->remove('capture_error');
				} else {
					$data['capture_error'] = '';
				}
	...
	public function void() {
		...
		if ($this->request->post['status'] == 'FAILED') {
			if (isset($this->request->post['fail_reason'])) {
				session()->set('void_error', $this->request->post['fail_reason']);
			} else {
				session()->set('void_error', $this->language->get('error_void_error'));
			}
		}

		if ($this->request->post['status'] == 'DECLINED') {
			session()->set('void_success', $this->language->get('success_void'));
		}

		$this->response->redirect($this->url->link('sale/order/info', 'order_id=' . $this->request->post['order_id'] . '&user_token=' . session('user_token'), true));
	}
	...
	public function capture() {
		...
		if ($this->request->post['status'] == 'FAILED') {
			if (isset($this->request->post['fail_reason'])) {
				session()->set('capture_error', $this->request->post['fail_reason']);
			} else {
				session()->set('capture_error', $this->language->get('error_capture_error'));
			}
		}

		if ($this->request->post['status'] == 'APPROVED') {
			session()->set('capture_success', $this->language->get('success_capture'));
		}
