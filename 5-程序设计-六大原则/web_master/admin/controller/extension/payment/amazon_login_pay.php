...
/**
 * @property ModelExtensionPaymentAmazonLoginPay $model_extension_payment_amazon_login_pay
	...
	public function index() {
		...
		if ((request()->isMethod('POST')) && $this->validate()) {
			...
			if (isset($this->request->post['language_reload'])) {
				$this->response->redirect($this->url->link('payment/amazon_login_pay', 'user_token=' . session('user_token'), true));
			} else {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/amazon_login_pay', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/payment/amazon_login_pay', 'user_token=' . session('user_token'), true);
