 ...
 * @property ModelExtensionPaymentDivido $model_extension_payment_divido
...
class ControllerExtensionPaymentDivido extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/divido', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/payment/divido', 'user_token=' . session('user_token'), 'SSL');
		
		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=payment', 'SSL');
