...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/klarna_checkout', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/payment/klarna_checkout', 'user_token=' . session('user_token'), true);
		...
		if (request()->isMethod('POST') && !isset($this->request->post['klarna_checkout_account'])) {
			$data['klarna_checkout_account'] = array();
		} elseif (request()->isMethod('POST') && isset($this->request->post['klarna_checkout_account'])) {
        ...
        $data['store_url'] = request()->serverBag->get('HTTPS') ? HTTPS_CATALOG : HTTP_CATALOG;
		...
		if ($api_info) {
            ...
            $data['api_ip'] = request()->serverBag->get('REMOTE_ADDR', '');
		} else {
			$data['api_id'] = '';
			$data['api_key'] = '';
			$data['api_ip'] = '';
