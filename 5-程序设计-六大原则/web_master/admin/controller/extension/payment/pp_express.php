...
/**
 * @property ModelExtensionPaymentPPExpress $model_extension_payment_pp_express
...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function index() {
		...
		$data['button_configure'] = $this->url->link('extension/module/pp_button/configure', 'user_token=' . session('user_token'), true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/pp_express', 'user_token=' . session('user_token'), true),
		);

		$data['action'] = $this->url->link('extension/payment/pp_express', 'user_token=' . session('user_token'), true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=payment', true);

		$data['search'] = $this->url->link('extension/payment/pp_express/search', 'user_token=' . session('user_token'), true);
		...
		$post_data = array(
			'return_url' => $this->url->link('extension/payment/pp_express', 'user_token=' . session('user_token'), true),
		...
		if ($paypal_info) {
			...
			foreach ($results as $result) {
				$data['transactions'][] = array(
					...
					'view'           => $this->url->link('extension/payment/pp_express/info', 'user_token=' . session('user_token') . '&transaction_id=' . $result['transaction_id'], true),
					'refund'         => $this->url->link('extension/payment/pp_express/refund', 'user_token=' . session('user_token') . '&transaction_id=' . $result['transaction_id'], true),
					'resend'         => $this->url->link('extension/payment/pp_express/resend', 'user_token=' . session('user_token') . '&paypal_order_transaction_id=' . $result['paypal_order_transaction_id'], true)
				);
			}
		}
	...
	public function refund() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express/refund', 'user_token=' . session('user_token'), true),
		);

		//button actions
		$data['action'] = $this->url->link('extension/payment/pp_express/doRefund', 'user_token=' . session('user_token'), true);
		$data['cancel'] = $this->url->link('extension/payment/pp_express', 'user_token=' . session('user_token'), true);
	...
	public function doRefund() {
		...
		if (isset($this->request->post['transaction_id']) && isset($this->request->post['refund_full'])) {
			...
			if ($this->request->post['refund_full'] == 0 && $this->request->post['amount'] == 0) {
				session()->set('error', $this->language->get('error_partial_amt'));
			} else {
				...
				if ($paypal_order) {
					...
					if ($result == false) {
						...
						$this->response->redirect($this->url->link('sale/order/info', 'user_token=' . session('user_token') . '&order_id=' . $paypal_order['order_id'], true));
						...
						session()->set('error', (isset($result['L_SHORTMESSAGE0']) ? $result['L_SHORTMESSAGE0'] : 'There was an error') . (isset($result['L_LONGMESSAGE0']) ? '<br />' . $result['L_LONGMESSAGE0'] : ''));
						$this->response->redirect($this->url->link('extension/payment/pp_express/refund', 'user_token=' . session('user_token') . '&transaction_id=' . $this->request->post['transaction_id'], true));
					}
				} else {
					session()->set('error', $this->language->get('error_data_missing'));
					$this->response->redirect($this->url->link('extension/payment/pp_express/refund', 'user_token=' . session('user_token') . '&transaction_id=' . $this->request->post['transaction_id'], true));
				}
			}
		} else {
			session()->set('error', $this->language->get('error_data'));
			$this->response->redirect($this->url->link('extension/payment/pp_express/refund', 'user_token=' . session('user_token') . '&transaction_id=' . $this->request->post['transaction_id'], true));
		}
	}
	...
	public function search() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express/search', 'user_token=' . session('user_token'), true),
		);
		...
		$data['view_link'] = $this->url->link('extension/payment/pp_express/info', 'user_token=' . session('user_token'), true);
	...
	public function info() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/payment/pp_express/info', 'user_token=' . session('user_token') . '&transaction_id=' . $this->request->get['transaction_id'], true),
		);
		...
		$data['view_link'] = $this->url->link('extension/payment/pp_express/info', 'user_token=' . session('user_token'), true);
		$data['cancel'] = $this->url->link('extension/payment/pp_express/search', 'user_token=' . session('user_token'), true);
		$data['user_token'] = session('user_token');
	...
	public function live() {
		if (isset($this->request->get['merchantId'])) {
			...
			if (isset($token->access_token)) {
				...
				session()->set('error_api', $this->language->get('error_api'));
			}
		...
		$this->response->redirect($this->url->link('extension/payment/pp_express', 'user_token=' . session('user_token'), true));
	}
	...
	public function sandbox() {
		if (isset($this->request->get['merchantId'])) {
			...
			if (isset($token->access_token)) {
				...
				session()->set('error_api', $this->language->get('error_api_sandbox'));
			}
	...
	public function connectRedirect() {
		if ($this->user->hasPermission('modify', 'extension/extension/payment') && $this->user->hasPermission('modify', 'extension/payment/pp_express')) {
			...
			if (isset($curl_response['url']) && !empty($curl_response['url'])) {
				...
				$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . session('user_token'), true));
			}
		} else {
			$this->response->redirect($this->url->link('error/permission', 'user_token=' . session('user_token'), true));
		}
	}
	...
	public function preferredSolution() {
		...
		if ($this->config->get('payment_pp_express_username') || $this->config->get('payment_pp_express_sandbox_username')) {
			$data['module_link'] = $this->url->link('extension/payment/pp_express', 'user_token=' . session('user_token'), true);
		} else {
			if ($this->user->hasPermission('modify', 'extension/extension/payment')) {
				$data['connect_link'] = $this->url->link('extension/payment/pp_express/connectRedirect', 'user_token=' . session('user_token'), true);
			}
		}
