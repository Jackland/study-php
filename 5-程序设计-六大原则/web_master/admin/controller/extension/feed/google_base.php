...
/**
 * @property ModelExtensionFeedGoogleBase $model_extension_feed_google_base
...
class ControllerExtensionFeedGoogleBase extends Controller {
	...
	public function index() {
		...
		if ((request()->isMethod('POST')) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=feed', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=feed', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/feed/google_base', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/feed/google_base', 'user_token=' . session('user_token'), true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=feed', true);
	...
	public function category() {
		...
		$pagination->limit = 10;
		$pagination->url = $this->url->link('extension/feed/google_base/category', 'user_token=' . session('user_token') . '&page={page}', true);
