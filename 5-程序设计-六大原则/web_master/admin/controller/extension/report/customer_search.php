 ...
 * @property ModelExtensionReportCustomer $model_extension_report_customer
...
class ControllerExtensionReportCustomerSearch extends Controller {
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/report/customer_search', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/report/customer_search', 'user_token=' . session('user_token'), true);
	...
	public function report() {
		...
		foreach ($results as $result) {
			...
			if ($result['customer_id'] > 0) {
				$customer = sprintf($this->language->get('text_customer'), $this->url->link('customer/customer/edit', 'user_token=' . session('user_token') . '&customer_id=' . $result['customer_id'], true), $result['customer']);
			} else {
		...
		$pagination->url = $this->url->link('report/report', 'user_token=' . session('user_token') . '&code=customer_search' . $url . '&page={page}', true);
