...
/**
 * @property ModelExtensionReportProduct $model_extension_report_product
...
class ControllerExtensionReportProductViewed extends Controller {
	public function index() {
		...
		if ((request()->isMethod('POST')) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=report', true));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=report', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('extension/report/product_viewed', 'user_token=' . session('user_token'), true)
		);

		$data['action'] = $this->url->link('extension/report/product_viewed', 'user_token=' . session('user_token'), true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . session('user_token') . '&type=report', true);
	...
	public function report() {
		...
		$data['reset'] = $this->url->link('extension/report/product_viewed/reset', 'user_token=' . session('user_token') . '&page={page}', true);
		...
		$pagination->url = $this->url->link('report/report', 'user_token=' . session('user_token') . '&code=product_viewed&page={page}', true);
	...
	public function reset() {
		...
		$this->response->redirect($this->url->link('report/report', 'user_token=' . session('user_token') . '&code=product_viewed' . $url, true));
	}
}