...
/**
 * @property ModelCustomerpartnerAddshipping $model_customerpartner_addshipping
 */
class ControllerCustomerpartnerShipping extends Controller {
	...
	protected function getList() {
		...
		$this->load->language('customerpartner/shipping');
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token'). $url, true),
      		'separator' => ' :: '
   		);

		$this->data['delete'] = $this->url->link('customerpartner/shipping/delete', 'user_token=' . session('user_token') , true);
    	$this->data['addshipping'] = $this->url->link('customerpartner/addshipping', 'user_token=' . session('user_token') , true);
		...
		$this->data['sort_name'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=name' . $url, true);
		$this->data['sort_country_code'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=cs.country_code' . $url, true);
		$this->data['sort_price'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=cs.price' . $url, true);
		$this->data['sort_zip_to'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=cs.zip_to' . $url, true);
		$this->data['sort_zip_from'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=cs.zip_from' . $url, true);
		$this->data['sort_weight_to'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=cs.weight_to' . $url, true);
		$this->data['sort_weight_from'] = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . '&sort=cs.weight_from' . $url, true);
		...
		$pagination->url = $this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . $url . '&page={page}', true);
  	...
  	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			$this->response->redirect($this->url->link('customerpartner/shipping', 'user_token=' . session('user_token') . $url, true));
		}
