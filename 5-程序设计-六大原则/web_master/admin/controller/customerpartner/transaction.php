...
class ControllerCustomerpartnerTransaction extends Controller {
	...
	private $data = array();
  	...
  	public function getlist() {
		...
		foreach ($filter_array as $unsetKey => $key) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('common/home', 'user_token=' . session('user_token'). $url, true),
      		'separator' => false
   		);

   		$this->data['breadcrumbs'][] = array(
       		'text'      => $this->language->get('heading_title'),
			'href'      => $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token'). $url, true),
      		'separator' => ' :: '
   		);

		$this->data['delete'] = $this->url->link('customerpartner/transaction/delete', 'user_token=' . session('user_token') , true);
		$this->data['insert'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token') , true);
	    ...
	    foreach ($results as $result) {
	      	...
	      	$this->data['transactions'][] = array(
				...
				'invoice' => $this->url->link('customerpartner/transaction/invoice', 'user_token=' . session('user_token') . '&transaction_id=' . $result['id'], true)
			);

		}

 		$this->data['user_token'] = session('user_token');
		...
		$this->data['sort_name'] = $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . '&sort=c.firstname' . $url, true);
		$this->data['sort_id'] = $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . '&sort=ct.id' . $url, true);
		$this->data['sort_date'] = $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . '&sort=ct.date_added' . $url, true);
		$this->data['sort_details'] = $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . '&sort=ct.details' . $url, true);
		$this->data['sort_amount'] = $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . '&sort=ct.amount' . $url, true);
		...
		$pagination->url = $this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . $url . '&page={page}', true);
  	...
  	public function addtransaction() {
		...
		$this->data['currency_code'] = $this->currency->getSymbolLeft(session('currency'));
    	...
    	$this->data['cancel'] = $this->url->link('customerpartner/income', 'user_token=' . session('user_token') , true);
		$this->data['save'] = $this->url->link('customerpartner/transaction/transactionSave', 'user_token=' . session('user_token') . $url, true);
		...
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token') . '&page={page}' . '&seller_id='.$this->request->get['seller_id'], true);
		...
		$this->data['limit_ten'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token') .'&sort_limit=10' . $url , true);

		$this->data['limit_twentyfive'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . '&sort_limit=25' . $url, true);

		$this->data['limit_fifty'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . '&sort_limit=50' . $url, true);

		$this->data['limit_hundred'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . '&sort_limit=100' . $url, true);

		$this->data['order_id_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token') . "&sort_order=op.order_id&sort=". $sort . $url , true);

		$this->data['product_name_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=o.firstname&sort=". $sort . $url, true);

		$this->data['payable_amount_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=c2o.customer&sort=". $sort . $url, true);

		$this->data['quantity_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=op.quantity&sort=". $sort . $url, true);

		$this->data['date_added_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=o.date_added&sort=". $sort . $url , true);

		$this->data['order_status_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=os.name&sort=". $sort . $url, true);

		$this->data['paid_status_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=c2o.paid_status&sort=". $sort . $url, true);

		$this->data['commission_applied_url'] = $this->url->link('customerpartner/transaction/addtransaction', 'user_token=' . session('user_token')  . "&sort_order=c2o.commission_applied&sort=". $sort . $url, true);
	...
	public function transactionSave() {
    	...
    	if ((request()->isMethod('POST')) && $this->validate()) {
			...
			if(!isset($this->error['warning'])){
				...
				$this->response->redirect($this->url->link('customerpartner/transaction', 'user_token=' . session('user_token'), true));
			}
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			...
			$this->response->redirect($this->url->link('customerpartner/transaction', 'user_token=' . session('user_token') . $url, true));
		}
