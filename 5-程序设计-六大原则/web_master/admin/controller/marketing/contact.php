...
class ControllerMarketingContact extends Controller {
	...
	public function index() {
		...
		$data['cancel'] = $this->url->link('marketing/contact', 'user_token=' . session('user_token'), true);
		...
		if (request()->isMethod('POST')) {
			...
			if (!$json) {
				...
				if ($emails) {
					...
					if ($end < $email_total) {
						$json['next'] = str_replace('&amp;', '&', $this->url->link('marketing/contact/send', 'user_token=' . session('user_token') . '&page=' . ($page + 1), true));
					} else {
						$json['next'] = '';
					}
