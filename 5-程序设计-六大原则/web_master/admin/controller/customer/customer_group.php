 ...
 * @property ModelSettingStore $setting/store
 */
class ControllerCustomerCustomerGroup extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('customer/customer_group', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('customer/customer_group', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('customer/customer_group/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('customer/customer_group/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['customer_groups'][] = array(
				...
				'edit'              => $this->url->link('customer/customer_group/edit', 'user_token=' . session('user_token') . '&customer_group_id=' . $result['customer_group_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('customer/customer_group', 'user_token=' . session('user_token') . '&sort=cgd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('customer/customer_group', 'user_token=' . session('user_token') . '&sort=cg.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('customer/customer_group', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
        ...
        $data['is_edit'] = isset($this->request->get['customer_group_id']) ? 1 : 0;
		if (isset($this->error['warning'])) {
			$data['error_warning'] = $this->error['warning'];
		} else {
			$data['error_warning'] = '';
		...
		if (!isset($this->request->get['customer_group_id'])) {
			$data['action'] = $this->url->link('customer/customer_group/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('customer/customer_group/edit', 'user_token=' . session('user_token') . '&customer_group_id=' . $this->request->get['customer_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('customer/customer_group', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['customer_group_id']) && (!request()->isMethod('POST'))) {
