...
class ControllerUserApi extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('user/api', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('user/api', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('user/api/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('user/api/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['apis'][] = array(
				...
				'edit'          => $this->url->link('user/api/edit', 'user_token=' . session('user_token') . '&api_id=' . $result['api_id'] . $url, true)
			);
		}
		...
		$data['sort_username'] = $this->url->link('user/api', 'user_token=' . session('user_token') . '&sort=username' . $url, true);
		$data['sort_status'] = $this->url->link('user/api', 'user_token=' . session('user_token') . '&sort=status' . $url, true);
		$data['sort_date_added'] = $this->url->link('user/api', 'user_token=' . session('user_token') . '&sort=date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->link('user/api', 'user_token=' . session('user_token') . '&sort=date_modified' . $url, true);
		...
		$pagination->url = $this->url->link('user/api', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
        ...
        $data['text_ip'] = sprintf($this->language->get('text_ip'), request()->serverBag->get('REMOTE_ADDR'));
		...
		if (!isset($this->request->get['api_id'])) {
			$data['action'] = $this->url->link('user/api/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('user/api/edit', 'user_token=' . session('user_token') . '&api_id=' . $this->request->get['api_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('user/api', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['api_id']) && (!request()->isMethod('POST'))) {
