...
class ControllerCatalogRecurring extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/recurring', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/recurring', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/recurring/add', 'user_token=' . session('user_token') . $url, true);
		$data['copy'] = $this->url->link('catalog/recurring/copy', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/recurring/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['recurrings'][] = array(
				...
				'edit'         => $this->url->link('catalog/recurring/edit', 'user_token=' . session('user_token') . '&recurring_id=' . $result['recurring_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/recurring', 'user_token=' . session('user_token') . '&sort=pd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/recurring', 'user_token=' . session('user_token') . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/recurring', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['recurring_id'])) {
			$data['action'] = $this->url->link('catalog/recurring/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/recurring/edit', 'user_token=' . session('user_token') . '&recurring_id=' . $this->request->get['recurring_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/recurring', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['recurring_id']) && (!request()->isMethod('POST'))) {
