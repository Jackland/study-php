...
class ControllerCatalogOption extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/option', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/option', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/option/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/option/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['options'][] = array(
				...
				'edit'       => $this->url->link('catalog/option/edit', 'user_token=' . session('user_token') . '&option_id=' . $result['option_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/option', 'user_token=' . session('user_token') . '&sort=od.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/option', 'user_token=' . session('user_token') . '&sort=o.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/option', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['option_id'])) {
			$data['action'] = $this->url->link('catalog/option/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/option/edit', 'user_token=' . session('user_token') . '&option_id=' . $this->request->get['option_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/option', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['option_id']) && (!request()->isMethod('POST'))) {
