...
class ControllerCatalogDownload extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/download', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/download', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/download/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/download/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['downloads'][] = array(
				...
				'edit'        => $this->url->link('catalog/download/edit', 'user_token=' . session('user_token') . '&download_id=' . $result['download_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/download', 'user_token=' . session('user_token') . '&sort=dd.name' . $url, true);
		$data['sort_date_added'] = $this->url->link('catalog/download', 'user_token=' . session('user_token') . '&sort=d.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/download', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['download_id'])) {
			$data['action'] = $this->url->link('catalog/download/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/download/edit', 'user_token=' . session('user_token') . '&download_id=' . $this->request->get['download_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/download', 'user_token=' . session('user_token') . $url, true);

		$this->load->model('localisation/language');

		$data['languages'] = $this->model_localisation_language->getLanguages();

		if (isset($this->request->get['download_id']) && (!request()->isMethod('POST'))) {
