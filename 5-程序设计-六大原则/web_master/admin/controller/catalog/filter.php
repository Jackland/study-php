...
class ControllerCatalogFilter extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/filter', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/filter', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/filter/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/filter/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['filters'][] = array(
				...
				'edit'            => $this->url->link('catalog/filter/edit', 'user_token=' . session('user_token') . '&filter_group_id=' . $result['filter_group_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/filter', 'user_token=' . session('user_token') . '&sort=fgd.name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('catalog/filter', 'user_token=' . session('user_token') . '&sort=fg.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/filter', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['filter_group_id'])) {
			$data['action'] = $this->url->link('catalog/filter/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/filter/edit', 'user_token=' . session('user_token') . '&filter_group_id=' . $this->request->get['filter_group_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/filter', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['filter_group_id']) && (!request()->isMethod('POST'))) {
