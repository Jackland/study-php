...
class ControllerLocalisationCountry extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/country', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/country', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('localisation/country/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('localisation/country/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['countries'][] = array(
				...
				'edit'       => $this->url->link('localisation/country/edit', 'user_token=' . session('user_token') . '&country_id=' . $result['country_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/country', 'user_token=' . session('user_token') . '&sort=name' . $url, true);
		$data['sort_iso_code_2'] = $this->url->link('localisation/country', 'user_token=' . session('user_token') . '&sort=iso_code_2' . $url, true);
		$data['sort_iso_code_3'] = $this->url->link('localisation/country', 'user_token=' . session('user_token') . '&sort=iso_code_3' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/country', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['country_id'])) {
			$data['action'] = $this->url->link('localisation/country/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/country/edit', 'user_token=' . session('user_token') . '&country_id=' . $this->request->get['country_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/country', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['country_id']) && (!request()->isMethod('POST'))) {
        ...
        if (isset($this->request->post['currency_id'])) {
            ...
            $data['currency_id'] = $country_info['currency_id'];
        } else {
            $data['currency_id'] = '0';
        }
        ...
        if (isset($this->request->post['show_flag'])) {
            $data['show_flag'] = $this->request->post['show_flag'];
        } elseif (!empty($country_info)) {
            $data['show_flag'] = $country_info['show_flag'];
        } else {
            $data['show_flag'] = '1';
        }
