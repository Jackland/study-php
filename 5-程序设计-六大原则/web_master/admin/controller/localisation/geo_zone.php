...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/geo_zone', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/geo_zone', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('localisation/geo_zone/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('localisation/geo_zone/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['geo_zones'][] = array(
				...
				'edit'        => $this->url->link('localisation/geo_zone/edit', 'user_token=' . session('user_token') . '&geo_zone_id=' . $result['geo_zone_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/geo_zone', 'user_token=' . session('user_token') . '&sort=name' . $url, true);
		$data['sort_description'] = $this->url->link('localisation/geo_zone', 'user_token=' . session('user_token') . '&sort=description' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/geo_zone', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['geo_zone_id'])) {
			$data['action'] = $this->url->link('localisation/geo_zone/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/geo_zone/edit', 'user_token=' . session('user_token') . '&geo_zone_id=' . $this->request->get['geo_zone_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/geo_zone', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['geo_zone_id']) && (!request()->isMethod('POST'))) {
