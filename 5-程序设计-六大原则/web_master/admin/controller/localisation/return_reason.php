...
class ControllerLocalisationReturnReason extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/return_reason', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/return_reason', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('localisation/return_reason/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('localisation/return_reason/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['return_reasons'][] = array(
				...
				'name'             => $result['name'],
				'edit'             => $this->url->link('localisation/return_reason/edit', 'user_token=' . session('user_token') . '&return_reason_id=' . $result['return_reason_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/return_reason', 'user_token=' . session('user_token') . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/return_reason', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['return_reason_id'])) {
			$data['action'] = $this->url->link('localisation/return_reason/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/return_reason/edit', 'user_token=' . session('user_token') . '&return_reason_id=' . $this->request->get['return_reason_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/return_reason', 'user_token=' . session('user_token') . $url, true);
