...
class ControllerLocalisationTaxClass extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/tax_class', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/tax_class', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('localisation/tax_class/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('localisation/tax_class/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['tax_classes'][] = array(
				...
				'title'        => $result['title'],
				'edit'         => $this->url->link('localisation/tax_class/edit', 'user_token=' . session('user_token') . '&tax_class_id=' . $result['tax_class_id'] . $url, true)
			);
		}
		...
		$data['sort_title'] = $this->url->link('localisation/tax_class', 'user_token=' . session('user_token') . '&sort=title' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/tax_class', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['tax_class_id'])) {
			$data['action'] = $this->url->link('localisation/tax_class/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/tax_class/edit', 'user_token=' . session('user_token') . '&tax_class_id=' . $this->request->get['tax_class_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/tax_class', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['tax_class_id']) && (!request()->isMethod('POST'))) {
