...
class ControllerLocalisationStockStatus extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/stock_status', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/stock_status', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('localisation/stock_status/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('localisation/stock_status/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['stock_statuses'][] = array(
				...
				'name'            => $result['name'],
				'edit'            => $this->url->link('localisation/stock_status/edit', 'user_token=' . session('user_token') . '&stock_status_id=' . $result['stock_status_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('localisation/stock_status', 'user_token=' . session('user_token') . '&sort=name' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/stock_status', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['stock_status_id'])) {
			$data['action'] = $this->url->link('localisation/stock_status/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/stock_status/edit', 'user_token=' . session('user_token') . '&stock_status_id=' . $this->request->get['stock_status_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/stock_status', 'user_token=' . session('user_token') . $url, true);
