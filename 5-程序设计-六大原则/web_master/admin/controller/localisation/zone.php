...
class ControllerLocalisationZone extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('localisation/zone', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('localisation/zone', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('localisation/zone/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('localisation/zone/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['zones'][] = array(
				...
				'code'    => $result['code'],
				'edit'    => $this->url->link('localisation/zone/edit', 'user_token=' . session('user_token') . '&zone_id=' . $result['zone_id'] . $url, true)
			);
		}
		...
		$data['sort_country'] = $this->url->link('localisation/zone', 'user_token=' . session('user_token') . '&sort=c.name' . $url, true);
		$data['sort_name'] = $this->url->link('localisation/zone', 'user_token=' . session('user_token') . '&sort=z.name' . $url, true);
		$data['sort_code'] = $this->url->link('localisation/zone', 'user_token=' . session('user_token') . '&sort=z.code' . $url, true);
		...
		$pagination->url = $this->url->link('localisation/zone', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['zone_id'])) {
			$data['action'] = $this->url->link('localisation/zone/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('localisation/zone/edit', 'user_token=' . session('user_token') . '&zone_id=' . $this->request->get['zone_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('localisation/zone', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['zone_id']) && (!request()->isMethod('POST'))) {
