...
class ControllerSaleReturn extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/return', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/return', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('sale/return/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('sale/return/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
				...
				'edit'          => $this->url->link('sale/return/edit', 'user_token=' . session('user_token') . '&return_id=' . $result['return_id'] . $url, true)
			);
		}
		...
		$data['sort_return_id'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=r.return_id' . $url, true);
		$data['sort_order_id'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=r.order_id' . $url, true);
		$data['sort_customer'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=customer' . $url, true);
		$data['sort_product'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=r.product' . $url, true);
		$data['sort_model'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=r.model' . $url, true);
		$data['sort_status'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=status' . $url, true);
		$data['sort_date_added'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=r.date_added' . $url, true);
		$data['sort_date_modified'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . '&sort=r.date_modified' . $url, true);
		...
		$pagination->url = $this->url->link('sale/return', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['return_id'])) {
			$data['action'] = $this->url->link('sale/return/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/return/edit', 'user_token=' . session('user_token') . '&return_id=' . $this->request->get['return_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/return', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['return_id']) && (!request()->isMethod('POST'))) {
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/return/history', 'user_token=' . session('user_token') . '&return_id=' . $this->request->get['return_id'] . '&page={page}', true);
