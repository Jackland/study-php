...
class ControllerSaleVoucher extends Controller {
	...
	public function add() {
		...
		if ((request()->isMethod('POST')) && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('sale/voucher', 'user_token=' . session('user_token') . $url, true));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('sale/voucher', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('sale/voucher/add', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('sale/voucher/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			if ($result['order_id']) {
				$order_href = $this->url->link('sale/order/info', 'user_token=' . session('user_token') . '&order_id=' . $result['order_id'] . $url, true);
			} else {
				$order_href = '';
			}
			...
			$data['vouchers'][] = array(
				...
				'edit'       => $this->url->link('sale/voucher/edit', 'user_token=' . session('user_token') . '&voucher_id=' . $result['voucher_id'] . $url, true),
				'order'      => $order_href
			);
		}
		...
		$data['sort_code'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=v.code' . $url, true);
		$data['sort_from'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=v.from_name' . $url, true);
		$data['sort_to'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=v.to_name' . $url, true);
		$data['sort_theme'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=theme' . $url, true);
		$data['sort_amount'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=v.amount' . $url, true);
		$data['sort_status'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=v.status' . $url, true);
		$data['sort_date_added'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . '&sort=v.date_added' . $url, true);
		...
		$pagination->url = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['voucher_id'])) {
			$data['action'] = $this->url->link('sale/voucher/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('sale/voucher/edit', 'user_token=' . session('user_token') . '&voucher_id=' . $this->request->get['voucher_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('sale/voucher', 'user_token=' . session('user_token') . $url, true);

		if (isset($this->request->get['voucher_id']) && (!!request()->isMethod('POST'))) {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $voucher_id) {
			...
			if ($order_voucher_info) {
				$this->error['warning'] = sprintf($this->language->get('error_order'), $this->url->link('sale/order/info', 'user_token=' . session('user_token') . '&order_id=' . $order_voucher_info['order_id'], true));
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/voucher/history', 'user_token=' . session('user_token') . '&voucher_id=' . $this->request->get['voucher_id'] . '&page={page}', true);
