...
/**
 * Class ModelExtensionDashboardSale
 */
class ModelExtensionDashboardSale extends Model {
	public function getTotalSales($data = array()) {
		$sql = "SELECT SUM(CASE WHEN (oc.country_id=107 AND oo.date_added>'2019-06-02 00:00:00') THEN oo.total/100 ELSE oo.total END ) AS total FROM `" . DB_PREFIX . "order` oo LEFT JOIN oc_customer oc ON oc.customer_id=oo.customer_id WHERE oo.order_status_id > '0'";

		if (!empty($data['filter_date_added'])) {
			$sql .= " AND DATE(oo.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}
