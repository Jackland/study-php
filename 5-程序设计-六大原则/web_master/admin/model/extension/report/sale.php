...
/**
 * Class ModelExtensionReportSale
 */
class ModelExtensionReportSale extends Model {
	...
	public function getOrders($data = array()) {
              ...
              SUM((SELECT SUM(op.quantity) FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = o.order_id GROUP BY op.order_id)) AS products,
               ...
               CASE
                    WHEN
                        oc.country_id = 107
                     THEN
                        (ifnull(opq.price,oop.price)+oop.service_fee/oop.quantity)*oop.quantity / 105
                    WHEN
                        oc.country_id = 81
                    THEN
                        (ifnull(opq.price,oop.price)+oop.service_fee/oop.quantity)*oop.quantity / 0.9
                    WHEN
                        oc.country_id = 222
                    THEN
                        (ifnull(opq.price,oop.price)+oop.service_fee/oop.quantity)*oop.quantity / 0.81
                    ELSE
                        (ifnull(opq.price,oop.price)+oop.service_fee/oop.quantity)*oop.quantity
                    END )
               AS `total`
               FROM `" . DB_PREFIX . "order` o
               LEFT JOIN oc_order_product oop ON o.order_id=oop.order_id
               LEFT JOIN oc_customer oc on o.customer_id = oc.customer_id
               LEFT JOIN oc_customerpartner_to_order cto ON cto.order_product_id = oop.order_product_id
              LEFT JOIN oc_product_quote opq ON opq.order_id=o.order_id and opq.product_id=oop.product_id
               ";
		...
		$sql .= " and (
                            (
                    o.customer_id = 105
                    AND cto.customer_id IN (31)
                    AND DATE(o.date_added) >='2019-08-15'
                )
                OR
                (
                    o.customer_id = 105
                    AND cto.customer_id not IN (32,31)
                    AND DATE(o.date_added) <'2019-08-15'
                )
            OR
                (
                    o.customer_id = 105
                    AND cto.customer_id not IN (32)
                    AND DATE(o.date_added) >='2019-08-15'
                )
                OR (
                    o.customer_id = 218
                    AND cto.customer_id NOT IN (31,32)
                )
                OR o.customer_id NOT IN (105, 218)
                 )
                 and (cto.customer_id !=29 or DATE(o.date_added) >= '2019-04-20' or o.order_id in(142,143))";
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_order_status_id'])) {
			...
			$sql .= " WHERE order_status_id > ".OcOrderStatus::TO_BE_PAID;
		}
	...
	public function getTaxes($data = array()) {
		...
		if (!empty($data['filter_order_status_id'])) {
			...
			$sql .= " AND o.order_status_id > ".OcOrderStatus::TO_BE_PAID;
		}
	...
	public function getTotalShipping($data = array()) {
		...
		if (!empty($data['filter_order_status_id'])) {
			...
			$sql .= " AND order_status_id > ".OcOrderStatus::TO_BE_PAID;
		}
