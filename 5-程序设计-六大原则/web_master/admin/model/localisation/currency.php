...
class ModelLocalisationCurrency extends Model {
	public function addCurrency($data) {
        ...
        $this->db->query("INSERT INTO tb_sys_exchange_rate_history (currency_id, new_exchange_rate, date_added, operator_id) values ('" . (int)$currency_id . "', '" . $this->db->escape($data['value']) . "',NOW(), " .$this->user->getId(). ")");
	...
	public function editCurrency($currency_id, $data) {
	    $oldCurrency = $this->getCurrency($currency_id);
        $currency_value = $oldCurrency['value'];
        $new_currency = $this->db->escape($data['value']);
        $fluctuation = abs(round(($new_currency-$currency_value)/$currency_value,4));
        $result = array();
        if($fluctuation>0.06){
            $this->cache->delete('currency');
            $result = array(
                'success'=>false,
                'msg' => 'Wider fluctuation margin of exchange rate，please check it.'
            );
            return $result;
        }else {
            if (isset($currency_value) && $currency_value != 0 && $currency_value != $this->db->escape($data['value'])) {
                $this->db->query("INSERT INTO tb_sys_exchange_rate_history (currency_id, old_exchange_rate, new_exchange_rate, date_added, operator_id,ip)
            values ('" . (int)$currency_id . "', " . $currency_value . ",'" . $this->db->escape($data['value']) . "',NOW(), " . $this->user->getId() . ",'" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "')");
            }
     ...
     * @return array
     */
	public function getCurrenciesHistory($currency_id,$pageNum){
        $bulid = $this->orm->table('tb_sys_exchange_rate_history')
            ->where('currency_id','=',$currency_id)
            ->orderBy('date_added','desc');
        $total = $bulid->count();
        $result = $bulid->forPage($pageNum, 20)->get();
        $resultArr = array(
            'currencyData'=>obj2array($result),
            'total' => $total
        );
        return $resultArr;
    }
