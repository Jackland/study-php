...
/**
 * Class ModelCustomerpartnerOrder
...
class ModelCustomerpartnerOrder extends Model {
    ...
    public function getOrders($data = array()) {
        ...
        if (isset($data['filter_order_status'])) {
            ...
            $sql .= " WHERE o.order_status_id > ".OcOrderStatus::TO_BE_PAID;
        }
    ...
    public function queryPurchaseOrderTotal($condition):Builder
    {
        $co = new Collection($condition);
        return $this->orm->table('oc_order as o')
            ->rightJoin('oc_customerpartner_to_order as c2o','c2o.order_id','=','o.order_id')
            ->leftJoin('oc_customer as c','c2o.customer_id','=','c.customer_id')
            ->when(
                (empty($co->get('filter_date_added'))
                    && empty($co->get('filter_date_modified'))
                    && empty($co->get('filter_order_id'))),function (Builder $q) {
                $q->whereRaw(new Expression('o.order_status_id > '.OcOrderStatus::TO_BE_PAID.' and o.date_added >DATE_SUB(CURDATE(), INTERVAL 3 Month)'));
            })
            ->when(!empty($co->get('filter_order_id')), function (Builder $q) use ($co) {
                $q->where('o.order_id', '=',  $co->get('filter_order_id') );
            })
            ->when(!empty($co->get('filter_customer')), function (Builder $q) use ($co) {
                $q->where(new Expression("CONCAT(c.firstname, ' ', c.lastname)"), 'LIKE', '%' . $co->get('filter_customer') . '%');
            })
            ->when($co->get('filter_order_status') != '', function (Builder $q) use ($co) {
                $q->where('o.order_status_id','=' ,$co->get('filter_order_status') );
            })
            ->when($co->get('filter_order_access') != '', function (Builder $q) use ($co) {
                if($co->get('filter_order_access') == 1){
                    $q->where('c2o.seller_access', '=',  $co->get('filter_order_access') );
                }else{
                    $q->where('c2o.seller_access', '=',  $co->get('filter_order_access'));
                }
            })
            ->when(!empty($co->get('filter_total')), function (Builder $q) use ($co) {
                $q->havingRaw(new Expression('round(SUM(c2o.price),2) = '.(float)$co->get('filter_total')));
            })
            ->when(!empty($co->get('filter_date_added')), function (Builder $q) use ($co) {
                $q->whereRaw(new Expression("DATE(o.date_added) = DATE('" . $co->get('filter_date_added') . "')"));
            })
            ->when(!empty($co->get('filter_date_modified')), function (Builder $q) use ($co) {
                $q->whereRaw(new Expression("DATE(o.date_modified) = DATE('" . $co->get('filter_date_modified') . "')"));
            })
            ->groupBy(['c2o.customer_id', 'c2o.order_id']);

    }

    public function queryPurchaseOrder($condition):Builder
    {
        $co = new Collection($condition);
        return $this->orm->table('oc_order as o')
            ->rightJoin('oc_customerpartner_to_order as c2o','c2o.order_id','=','o.order_id')
            ->leftJoin('oc_customer as c','c2o.customer_id','=','c.customer_id')
            ->leftJoin('oc_order_status as os',function(Builder $join){
                $join->on('os.order_status_id', '=', 'o.order_status_id')
                    ->where([
                        ['os.language_id', '=', $this->config->get('config_language_id')],
                    ]);
            })
            ->when((empty($co->get('filter_date_added'))
                && empty($co->get('filter_date_modified'))
                && empty($co->get('filter_order_id'))),function (Builder $q) {
                $q->whereRaw(new Expression('o.order_status_id > '.OcOrderStatus::TO_BE_PAID.' and o.date_added >DATE_SUB(CURDATE(), INTERVAL 3 Month)'));
            })
    ...
    public function getPurchaseOrderList($condition)
    {
        $ret = $this->queryPurchaseOrder(...func_get_args());
        $ret = $ret->orderBy($condition['sort'],$condition['order']);
        if (isset($condition['page']) && isset($condition['page_limit'])) {
            $ret->forPage(($condition['page'] ?? 1), ($condition['page_limit'] ?? 20));
        }
        return $ret
            ->selectRaw("o.order_id,
                    c2o.customer_id AS seller_id,
     ...
     * @return int
     */
    public function getPurchaseOrderTotal($condition)
    {
        $query = get_complete_sql($this->queryPurchaseOrderTotal(...func_get_args())->selectRaw('o.order_id'));
        return $this->orm
            ->table(new Expression('(' . $query . ') as a'))
            ->count();
    }
    ...
    public function getTotalOrdersByOrderStatusId($order_status_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . (int)$order_status_id . "' AND order_status_id > ".OcOrderStatus::TO_BE_PAID);
    ...
    public function getTotalOrdersByLanguageId($language_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE language_id = '" . (int)$language_id . "' AND order_status_id > ".OcOrderStatus::TO_BE_PAID);

        return $query->row['total'];
    }

    public function getTotalOrdersByCurrencyId($currency_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE currency_id = '" . (int)$currency_id . "' AND order_status_id > ".OcOrderStatus::TO_BE_PAID);
     ...
     * @param int $order_id
     * @param int $admin_cancel_order_status
     ...
     * @param string $seller_change_order_status_name
     * @return bool
     */
