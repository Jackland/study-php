...
class ReceiptCheckForm extends RequestForm
{
    /**
     * 产品id
     * @var $productId
    ...
    public function getData(): array
    {
        ...
        $query = db('tb_sys_receipts_order as ro')
            ...
            ->where('ro.receive_number', $this->keyword)
            ...
            $tempQuery = ReceiptsOrderDetail::query()
                ...
                ->whereExists(function ($q) {
                    $q->select('*')
                        ...
                        ->whereRaw('sb.receipts_order_line_id = rod.receive_order_detail_id')
                        ->where('sb.customer_id', $this->customerId)
                        ->where('sb.onhand_qty', '>', 0);
                });
            $query = $query->whereIn('ro.receive_order_id', $tempQuery);
        }
        $res = db(new Expression('(' . get_complete_sql($query) . ') as t'))->value('receive_order_id');

        return $res ? ['receive_order_id' => $res] : [];
    }
}
