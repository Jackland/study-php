 ...
 * @property ModelExtensionPaymentSagePayServer $model_extension_payment_sagepay_server
 */
class ControllerExtensionPaymentSagepayServer extends Controller {
	...
	public function send() {
		...
		$payment_data['VendorTxCode'] = session('order_id') . 'T' . strftime("%Y%m%d%H%M%S") . mt_rand(1, 999);
	...
	public function failure() {
		...
		session()->set('error', $this->language->get('text_generic_error'));
