 ...
 * @property ModelExtensionPaymentKlarnaCheckout $model_extension_payment_klarna_checkout
...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function main() {
		...
		if ((!isset($this->session->data['shipping_method']) || empty($this->session->data['shipping_method'])) && (isset($this->session->data['shipping_methods']) && !empty($this->session->data['shipping_methods']))) {
			session()->set('shipping_method', $this->model_extension_payment_klarna_checkout->getDefaultShippingMethod($this->session->data['shipping_methods']));
		}
		...
		if (!$redirect) {
			...
			if (isset($this->session->data['klarna_checkout_currency']) && $this->session->data['klarna_checkout_currency'] != $this->session->data['currency']) {
				...
				$this->session->remove('klarna_checkout_order_id');
				$this->session->remove('klarna_checkout_data');
			}

			session()->set('klarna_checkout_currency', session('currency'));
			...
			if ($klarna_checkout) {
				session()->set('klarna_checkout_order_id', $klarna_checkout['order_id']);
	...
	public function sidebar() {
		...
		if (isset($this->session->data['shipping_methods'])) {
			$data['shipping_methods'] = session('shipping_methods');
		} else {
	...
	public function shippingAddress() {
		...
		if ((request()->isMethod('POST')) && isset($this->request->post['country'])) {
	...
	public function addressUpdate() {
		...
		if ($process) {
			...
			session()->set('currency', $order_info['currency_code']);
			session()->set('language', $order_info['language_code']);
					...
					if ($shipping_methods) {
						...
						if ($klarna_account && $connector) {
							...
							if ($this->cart->hasShipping()) {
								...
								if (!$shipping_method) {
									session()->set('shipping_method', $this->model_extension_payment_klarna_checkout->getDefaultShippingMethod($shipping_methods));
								}
	...
	private function setPayment() {
		...
		if (isset($this->session->data['payment_address']) && !empty($this->session->data['payment_address'])) {
			session()->set('payment_address', session('payment_address'));
	...
	private function setShipping() {
		...
		if (isset($this->session->data['shipping_address']) && !empty($this->session->data['shipping_address'])) {
			session()->set('shipping_address', session('shipping_address'));
		} elseif ($this->customer->isLogged() && $this->customer->getAddressId()) {
		...
		if (isset($this->session->data['shipping_address'])) {
			...
			session()->set('shipping_methods', $method_data);
		}
	}
	...
	private function createOrder() {
		...
		$order_data['comment'] = session('comment');
		...
		session()->set('order_id', $this->model_checkout_order->addOrder($order_data));
	}
	...
	private function klarnaOrderData($klarna_account) {
		...
		$klarna_order_data['merchant_reference1'] = session('order_id');
