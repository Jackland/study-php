...
class ControllerExtensionPaymentPPExpress extends Controller {
	...
	public function express() {
		...
		if (!isset($result['TOKEN'])) {
			session()->set('error', $result['L_LONGMESSAGE0']);
			/**
	...
	public function expressReturn() {
		...
		if (isset($result['PAYMENTREQUEST_0_NOTETEXT'])) {
			session()->set('comment', $result['PAYMENTREQUEST_0_NOTETEXT']);
		}
		...
		if ($this->session->data['paypal']['guest'] == true) {
			...
			if ($this->cart->hasShipping()) {
				...
				session()->set('shipping_postcode', $result['PAYMENTREQUEST_0_SHIPTOZIP']);
				...
				if ($country_info) {
					...
					session()->set('shipping_country_id', $country_info['country_id']);
				} else {
					...
					session()->set('shipping_country_id', '');
				}
				...
				if ($zone_info) {
					...
					session()->set('shipping_zone_id', $zone_info['zone_id']);
				} else {
			...
			if ($this->cart->hasShipping()) {
				...
				foreach($addresses as $address) {
					if (trim(strtolower($address['address_1'])) == trim(strtolower($result['PAYMENTREQUEST_0_SHIPTOSTREET'])) && trim(strtolower($address['postcode'])) == trim(strtolower($result['PAYMENTREQUEST_0_SHIPTOZIP']))) {
						...
						session()->set('payment_address_id', $address['address_id']);
						session()->set('payment_country_id', $address['country_id']);
						session()->set('payment_zone_id', $address['zone_id']);

						session()->set('shipping_address_id', $address['address_id']);
						session()->set('shipping_country_id', $address['country_id']);
						session()->set('shipping_zone_id', $address['zone_id']);
						session()->set('shipping_postcode', $address['postcode']);
				...
				if ($match == false) {
					...
					session()->set('payment_address_id', $address_id);
					session()->set('payment_country_id', $address_data['country_id']);
					session()->set('payment_zone_id', $address_data['zone_id']);

					session()->set('shipping_address_id', $address_id);
					session()->set('shipping_country_id', $address_data['country_id']);
					session()->set('shipping_zone_id', $address_data['zone_id']);
					session()->set('shipping_postcode', $address_data['postcode']);
				}
			} else {
				session()->set('payment_address_id', '');
				session()->set('payment_country_id', '');
				session()->set('payment_zone_id', '');
			}
		}
	...
	public function expressConfirm() {
		...
		if (isset($this->request->post['coupon']) && $this->validateCoupon()) {
			...
			session()->set('success', $this->language->get('text_coupon'));
		...
		if (isset($this->request->post['voucher']) && $this->validateVoucher()) {
			session()->set('voucher', $this->request->post['voucher']);

			session()->set('success', $this->language->get('text_voucher'));
		...
		if (isset($this->request->post['reward']) && $this->validateReward()) {
			session()->set('reward', abs($this->request->post['reward']));

			session()->set('success', $this->language->get('text_reward'));
			...
			if (!empty($shipping_address)) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						session()->set('shipping_methods', $quote_data);
						...
						if (!isset($this->session->data['shipping_method'])) {
							...
							session()->set('shipping_method', $quote_data[$key1]['quote'][$key2]);
						}
		...
		if (!isset($method_data['pp_express'])) {
			session()->set('error_warning', $this->language->get('error_unavailable'));
			$this->response->redirect($this->url->link('checkout/checkout', '', true));
		}

		session()->set('payment_methods', $method_data);
		session()->set('payment_method', $method_data['pp_express']);
	...
	public function expressComplete() {
		...
		if ($redirect == '') {
			...
			$data['currency_code'] = session('currency');
			...
			if ($result['ACK'] == 'Success') {
				...
				if ($recurring_products) {
					...
					foreach($recurring_products as $item) {
						...
						if ($item['recurring']['trial']) {
							...
							$trial_amt = $this->currency->format($this->tax->calculate($item['recurring']['trial_price'], $item['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], false, false) * $item['quantity'] . ' ' . session('currency');
						...
						$recurring_amt = $this->currency->format($this->tax->calculate($item['recurring']['price'], $item['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'], false, false)  * $item['quantity'] . ' ' . session('currency');
				...
				if ($result['L_ERRORCODE0'] == '10486') {
					if (isset($this->session->data['paypal_redirect_count'])) {
						...
						if ($this->session->data['paypal_redirect_count'] == 2) {
							...
							session()->set('error', $this->language->get('error_too_many_failures'));
							$this->response->redirect($this->url->link('checkout/checkout', '', true));
						} else {
							$this->session->data['paypal_redirect_count']++;
						}
					} else {
						session()->set('paypal_redirect_count', 1);
					}
				...
				session()->set('error_warning', $result['L_LONGMESSAGE0']);
				$this->response->redirect($this->url->link('extension/payment/pp_express/expressConfirm', '', true));
			}
		} else {
	...
	protected function shippingValidate($code) {
		...
		if (empty($code)) {
			session()->set('error_warning', $this->language->get('error_shipping'));
			return false;
		} else {
			...
			if (!isset($shipping[0]) || !isset($shipping[1]) || !isset($this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]])) {
				...
				session()->set('shipping_method', $this->session->data['shipping_methods'][$shipping[0]]['quote'][$shipping[1]]);
				session()->set('success', $this->language->get('text_shipping_updated'));
				return true;
			}
		}
	}
	...
	protected function validateCoupon() {
		...
		if ($coupon_info) {
			...
			session()->set('error_warning', $this->language->get('error_coupon'));
			return false;
		}
	}
	...
	protected function validateVoucher() {
		...
		if ($voucher_info) {
			...
			session()->set('error_warning', $this->language->get('error_voucher'));
			return false;
		}
	}
