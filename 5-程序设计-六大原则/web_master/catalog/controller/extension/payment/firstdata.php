...
class ControllerExtensionPaymentFirstdata extends Controller {
	public function index() {
		...
		$data['order_id'] = 'CON-' . session('order_id') . 'T' . $data['timestamp'] . mt_rand(1, 999);
	...
	public function fail() {
		...
		if (isset($this->request->post['fail_reason']) && !empty($this->request->post['fail_reason'])) {
			session()->set('error', $this->request->post['fail_reason']);
		} else {
			session()->set('error', $this->language->get('error_failed'));
		}
