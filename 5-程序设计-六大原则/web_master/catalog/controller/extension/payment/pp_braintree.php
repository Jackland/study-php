 ...
 * @property ModelExtensionPaymentPPBraintree $model_extension_payment_pp_braintree
...
class ControllerExtensionPaymentPPBraintree extends Controller {
	...
	public function index() {
		...
		$order_info = $this->model_checkout_order->getOrder(session('order_id'));
	...
	public function payment() {
		...
		if ($success) {
			...
			$this->model_checkout_order->addOrderHistory(session('order_id'), $order_status_id);
			...
			if ($transaction->success) {
				...
				session()->set('error', $this->language->get('error_process_order'));
		...
		if (isset($this->session->data['order_id'])) {
			$this->model_checkout_order->addOrderHistory(session('order_id'), $this->config->get('payment_pp_braintree_failed_id'));
		}
	...
	public function expressSetup() {
		...
		session()->set('paypal_braintree', $this->request->post);
	...
	public function expressConfirm() {
		...
		foreach ($products as $product) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				$price = $this->currency->format($unit_price, session('currency'));
				$total = $this->currency->format($unit_price * $product['quantity'], session('currency'));
			} else {
				$price = false;
				$total = false;
			}
		...
		if ($this->cart->hasShipping()) {
			...
			if ($this->customer->isLogged()) {
				$this->load->model('account/address');
				$shipping_address = $this->model_account_address->getAddress(session('shipping_address_id'));
		...
		if ($this->customer->isLogged() && isset($this->session->data['payment_address_id'])) {
			$this->load->model('account/address');
			$payment_address = $this->model_account_address->getAddress(session('payment_address_id'));
		} elseif (isset($this->session->data['guest'])) {
		...
		session()->set('payment_method', $method_data['pp_braintree']);
