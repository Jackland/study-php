...
/**
 * @property ModelExtensionTotalVoucher $model_extension_total_voucher
 */
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (isset($this->request->post['voucher'])) {
				...
				foreach ($this->request->post['voucher'] as $voucher) {
					if (isset($voucher['code']) && isset($voucher['to_name']) && isset($voucher['to_email']) && isset($voucher['from_name']) && isset($voucher['from_email']) && isset($voucher['voucher_theme_id']) && isset($voucher['message']) && isset($voucher['amount'])) {
						$this->session->data['vouchers'][$voucher['code']] = array(
							'code'             => $voucher['code'],
							'description'      => sprintf($this->language->get('text_for'), $this->currency->format($this->currency->convert($voucher['amount'], session('currency'), $this->config->get('config_currency')), session('currency')), $voucher['to_name']),
							...
							'amount'           => $this->currency->convert($voucher['amount'], session('currency'), $this->config->get('config_currency'))
						);
					}
				}
				...
				if (($this->request->post['amount'] < $this->config->get('config_voucher_min')) || ($this->request->post['amount'] > $this->config->get('config_voucher_max'))) {
					$json['error']['amount'] = sprintf($this->language->get('error_amount'), $this->currency->format($this->config->get('config_voucher_min'), session('currency')), $this->currency->format($this->config->get('config_voucher_max'), session('currency')));
				}
				...
				if (!$json) {
					...
					$this->session->data['vouchers'][$code] = array(
						'code'             => $code,
						'description'      => sprintf($this->language->get('text_for'), $this->currency->format($this->currency->convert($this->request->post['amount'], session('currency'), $this->config->get('config_currency')), session('currency')), $this->request->post['to_name']),
						...
						'amount'           => $this->currency->convert($this->request->post['amount'], session('currency'), $this->config->get('config_currency'))
					);
