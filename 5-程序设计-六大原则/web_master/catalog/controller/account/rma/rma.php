...
class ControllerAccountRmarma extends Controller {
	...
	public function index() {
		...
		if (!$this->customer->isLogged() AND !isset($this->session->data['rma_login'])) {
			session()->set('redirect', $this->urlChange('account/rma/rma', '', true));
	...
	private function getlist(){
	  ...
	  foreach ($results as $result) {
  		...
  		$data['rma_ress'][] = array(
				...
				'product' => $product,
				'reason' => $reason,
				'price' => $this->currency->format($result['value'],session('currency')),
				'date' => $result['date'],
				'color' => $result['color'],
		...
		if (isset($this->session->data['rma_error'])) {
			$this->error['warning'] = session('rma_error');
			$this->session->remove('rma_error');
		}
		...
		if (version_compare(VERSION, '2.2', '>=')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/rma/rma')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/rma/rma', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/account/rma/rma', $data));
			}
		}
		...
		if(isset($this->request->get['rma']) AND $this->request->get['rma']){
			if($this->customer->getId() || isset($this->session->data['rma_login'])){
				...
				if($results){
					...
					session()->set('success', $this->language->get('text_cancel_rma'));
				}else{
					session()->set('rma_error', $this->language->get('error_rma_delete'));
				}
			}else{
				session()->set('rma_error', $this->language->get('error_rma_delete'));
			}
	...
	public function getorder() {
		if(isset($this->request->post['order']) && $this->request->post['order']){
			...
			foreach ($products as $key => $product) {
				...
				$seller = $this->model_account_rma_rma->getSellerFromOrder((int)$this->request->post['order'],$product['order_product_id']);

				if (!isset($sellers[$seller])) {
					$sellers[$seller] = $this->model_account_rma_rma->getCustomerReason($seller);
				}
				$products[$key]['reasons'] = $sellers[$seller];
