...
class ControllerAccountRmaRmalogin extends Controller {
	...
	public function getlist(){
		...
		if (version_compare(VERSION, '2.2', '>=')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/rma/rmalogin')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/rma/rmalogin', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/account/rma/rmalogin', $data));
			}
		}
		...
		if ((request()->isMethod('POST'))) {
			...
			if(!isset($this->error['warning'])){
				...
				if($result AND $result['customer_id']==0){
					session()->set('rma_login', $data['email']);
					$this->response->redirect($this->urlChange('account/rma/rma', '', true));
