...
class ControllerAccountRmaviewrma extends Controller {
	...
	public function index() {
		...
		if (!$this->customer->isLogged() AND !isset($this->session->data['rma_login'])) {
			session()->set('redirect', $this->urlChange('account/rma/viewrma&vid='.$this->request->get['vid'], '', true));
	  ...
	  $this->document->setTitle($this->language->get('heading_title'));
		...
		if(!$result){
			session()->set('rma_error', $this->language->get('text_error'));
			$this->response->redirect($this->urlChange('account/rma/rma', '', true));
		}
			...
			if(isset($this->request->post['wk_view_message'])){
				...
				if($this->request->files['up_file']['name']){
					...
					if($this->fileValidation($file)){
						...
						session()->set('success', $this->language->get('text_success_msg'));
					}
				} else {
					$writer = 'me';
					$this->model_account_rma_rma->insertMessageRma($this->request->post['wk_view_message'],$writer,$this->request->post['wk_viewrma_grp_id'],$file_name);
				...
				if(isset($this->request->post['wk_view_reopensolved'])){
					...
					session()->set('success', $this->language->get('text_success_reopen'));
				}
	  ...
	  if ($result) {
	    	$this->document->addScript('catalog/view/javascript/RMA/ajaxfileupload.js?v='.APP_VERSION);
	    	...
	    	$data['prodetails'] = array();
	     	...
	     	if($prodetails){
	     		foreach($prodetails as $products){
	     			$data['prodetails'][]	= array(
							'price' => $this->currency->format($products['price'],session('currency')),
	     				'total' => $this->currency->format(($products['price'] + $products['tax'])*$products['returned'],session('currency')),
	     				'name' => $products['name'],
      ...
      $images = $this->getFolderImage($result['images']);
		...
		if (version_compare(VERSION, '2.2', '>=')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/rma/viewrma'.$tpl.'')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/rma/viewrma'.$tpl.'', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/account/rma/viewrma'.$tpl.'', $data));
			}
		}
	}
	...
	public function printlable() {
		...
		if (!$this->customer->isLogged() AND !isset($this->session->data['rma_login'])) {
			session()->set('redirect', $this->urlChange('account/rma/viewrma/printlable&vid='.$this->request->get['vid'], '', true));
			$this->response->redirect($this->urlChange('account/rma/rmalogin', '', true));
		}
		...
		if (version_compare(VERSION, '2.2', '>=')) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/rma/printlable')) {
				$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/rma/printlable', $data));
			} else {
				$this->response->setOutput($this->load->view('default/template/account/rma/printlable', $data));
			}
		}
	}
		...
		if($result && $result['images']){
			...
			if ($images) {
				...
				foreach ($images as $key => $image) {
					...
					$json['success'][$key]['resize'] = $this->model_tool_image->resize($img_folder_rma . '/' . $image['name'],125,125);
					$json['success'][$key]['thumb'] = $this->model_tool_image->resize($img_folder_rma . '/' . $image['name'],500,500);
				}
			} else {
				if (isset($this->error['warning'])) {
