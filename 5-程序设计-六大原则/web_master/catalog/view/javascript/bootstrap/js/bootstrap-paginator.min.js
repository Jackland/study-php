!function ($) {
  ...
  BootstrapPaginator.prototype = {
    init: function (element, options) {
      ...
      this.currentPage = 1, this.lastPage = 1, this.setOptions(options), this.initialized = !0, "function" == typeof this.onAfterInit && this.onAfterInit(this);
      ...
      let page_info = {};
      page_info[option.pageLimitKey] = option.rowsOfPage;
      event['page_info'] = page_info;
      // 全局覆盖分页信息
      let data = {};
      data[option.pageLimitKey] = option.rowsOfPage;
      $.ajaxSetup({data: data});
    ...
    }, onAfterInit: function (instance) {
      let page = $(instance.$element[0]);
      let container = page.parent();

      let option = instance.options;
      if (
        option.pageRange &&
        Array.isArray(option.pageRange) &&
        option.pageRange.length > 0 &&
        option.total &&
        option.total > 0
      ) {
        let page_range = option.pageRange;
        let append_span = $('<span></span>');
        append_span.attr('class', 'page-list');
        append_span.append('<b>Rows per page:&nbsp;</b>');
        let str = '';
        str += '<span class="btn-group dropup see">' +
          '<button class="btn btn-secondary dropdown-toggle" type="button" aria-expanded="true">' +
          '<span class="page-size">' + option.rowsOfPage +
          '</span><span class="caret"></span></button>';
        str += '<ul class="dropdown-menu" role="menu"><li>';
        page_range.forEach(function (val) {
          if (val == option.rowsOfPage) {
            str += '<a class="dropdown-item active" href="javascript:void(0);">' + val +
              '</a>';
          } else {
            str += '<a class="dropdown-item a" href="javascript:void(0);">' + val +
              '</a>';
          }
        });
        str += '</li></ul></span>';
        append_span.append(str);
        container.append(append_span);
      }
      if (option.total && option.total > 0) {
        let page = parseInt(option.currentPage);
        let page_limit = parseInt(option.rowsOfPage);
        let total = parseInt(option.total);
        let start_num = (page - 1) * page_limit + 1;
        let end_num = page * page_limit;
        end_num = total > end_num ? end_num : option.total;
        let str = '';
        str += '<span class="page-info">' + start_num + '-' + end_num + ' of ' + total + '</span>';
        container.append(str);
      }
      // 点击事件
      container.find('.dropdown-item').on('click', function () {
        let page_limit = parseInt($(this).html());
        let current_page_limit = parseInt(option.rowsOfPage);
        if (current_page_limit === page_limit) return;
        let data = {};
        data[option.pageLimitKey] = page_limit;
        $.ajaxSetup({data: data});
        instance.$element.trigger('page-clicked', [{page_info: data}, null, 1]);
      });
      // 全局覆盖分页信息
      let data = {};
      data[option.pageLimitKey] = option.rowsOfPage;
    ...
    itemTexts: function (type, page, current) {
      switch (type) {
        case"first":
          return '<i class="giga icon-left2 page-direction"></i>';
        case"prev":
          return '<i class="giga icon-arrow-left page-direction"></i>';
        case"next":
          return '<i class="giga icon-arrow-right-copy page-direction"></i>';
        case"last":
          return '<i class="giga icon-right2 page-direction"></i>';
        case"page":
          return page
      }
    },
