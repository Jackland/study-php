...
let WpFileUpload = function (containerId) {
  ...
  let wfu = {
    ...
    "beforeUpload":function(fileList){// 在上传前面执行的回调函数
    },
...
let WpFileUploadEvent = {
  ...
  "initWithSelectFile" : function(wfu){
    ...
    selectFileBtObj.css("background-color","#3a75dc");
    selectFileBtObj.removeClass('disabled');
    selectFileBtObj.off();
    selectFileBtObj.on("click",function(){
      if(wfu.autoCommit){
        WpFileUploadEvent.cleanFileEvent(wfu,0);
      }
  ...
  "initWithCleanFile" : function (wfu) {
    ...
    if(!wfu.isHiddenCleanBt){
      ...
      cleanBtObj.on("click",function(){
        ...
        WpFileUploadEvent.cleanFileEvent(wfu,1);
      });
      cleanBtObjIcon.css("color","#3a75dc");
    }
  },
  /**
   * 选择文件事件
   ...
   * @param wfu 操作的对象
   */
  "cleanFileEvent" : function(wfu,def = 0){
    let uploadId = wfu.uploadId;
    if(def == 1){
      getcleanFile(wfu);
    }
  ...
  "initWithUpload" : function(wfu){
    ...
    if(!wfu.isHiddenUploadBt){
      ...
      uploadFileBtIcon.css("color","#3a75dc");
    }
...
let WpFileUploadViewsModel = {
  ...
  "getHeadButtonsView" : function(wfu){
    // 选择文件的按钮标题
    let selectFileButtonTitle = 'choose file';
    let btsStr = '';
    ...
    // 清理按钮
    if(!wfu.isHiddenCleanBt){
      btsStr += '<div class="cleanFileBt none">';
      btsStr += '<i class="iconfont icon-shanchu"></i>';
      btsStr += '</div>';
    }
    btsStr += '</div>';
    return btsStr;
  ...
  "getFileItemModel":function(isImg,fileType,fileName,isImgUrl,fileCodeId){
    ...
    // modelStr += '</div>';
    // modelStr += '<div class="fileName" title="'+ fileName +'"  style="overflow: hidden;cursor:pointer">';
    //
    // modelStr += fileName;
    // modelStr += '</div>';
    // modelStr += '</div>';
  ...
  "startUploadFile" : function (wfu,formData,rememberFile) {
    $.ajax({
      ...
      success:function(data){
        ...
        // 自定清除文件
        if (wfu.isAutoClean){
          setTimeout(function () {WpFileUploadEvent.cleanFileEvent(wfu,0);},2000) ;
        }
...
let WpFileUploadTools = {
  ...
  "setProgressShow":function(uploadId, isShow){
    ...
    $("#"+uploadId+" .cleanFileBt").css("display", display);
  ...
  "addFileList" : function (fileList, wfu) {
    ...
    for(let i = 0; i < fileList.length; i++){
      ...
      if(isOutOfSize){
        WpFileUploadMessageModel.outOfSize(fileItem.name,fileSizeLimit);
        continue;
      }
      // 获取文件后缀
      ...
      if (fileIsInType) {
        ...
        WpFileUploadMessageModel.notSupperFileType(fileItem.name);
      }
    }
    // 设置文件
    WpFileUploadFileList.setFileList(fileListArray,wfu);
  ...
  "getFileUploadProgressMsg" : function (wfu) {
    ...
    if(wfu.showSummerProgress){
      ...
      $("#"+uploadId+" .cleanFileBt").css("display","block");
    ...
    if (!wfu.scheduleStandard && progressUrl !== "#" && progressUrl != null && progressUrl !== "") {
      ...
      if (wfu.scheduleStandard) {
        ...
        let speedSchedule = WpFileUploadComputer.div(filesDataAmount , 5);
        // 创建定时器，进行模拟进度演示
        let intervalId = setInterval(function(){
   ...
   */
  "showUploadProgress":function(wfu,bytesRead,percent){
  ...
  "disableFileSelect":function(wfu){
    ...
    selectFileBt.css("background-color","#ccc");
    selectFileBt.addClass('disabled')
    selectFileBt.off();
  },
  /**
   * 禁用文件上传
  ...
  "disableFileUpload":function(wfu){
    if (!wfu.isHiddenUploadBt) {
      ...
      let uploadFileBtIcon = $("#"+uploadId+" .uploadBts .uploadFileBt i");
      uploadFileBt.off();
      uploadFileBtIcon.css("color","#ccc");
    }
  },
  /**
   * 禁用文件清除
...
let WpFileUploadMessageModel = {
  ...
  "fileIsExit" : function (fileName) {
    ...
    return "The file has been uploaded and cannot be repeated.";
  },
  /**
   * 文件已经上传过了
   * @param fileName 文件名
  ...
  "outOfTotalSize" : function (totalSize) {
    ...
    return "The file loaded must not exceed 1MB.";
  },
  /**
   * 单个文件超出大小限定
   * @param fileName 文件名字
  ...
  "outOfSize" : function (fileName,fileSizeLimit) {
    ...
    // return "The file loaded must not exceed 1MB.";
    alert('The file loaded must not exceed 1MB.');
  },
  /**
   * 不支持文件类型错误
   * @param fileName 文件名称
  ...
  "notSupperFileType" : function (fileName) {
    // alert("文件格式错误：不支持该格式文件上传:"+fileName);
    alert("You can only upload PDF files.");
  },
  /**
   * 没有要上传的文件
   */
  "noFileUpload" : "no lable upload！",
  /**
