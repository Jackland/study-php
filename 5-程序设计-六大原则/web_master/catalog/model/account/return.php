...
class ModelAccountReturn extends Model {
	...
	public function getOrderIdAndProductIdByRmaOrderId($rma_order_id){
	    $result = $this->orm->table('oc_yzc_rma_order as ro')
            ->leftJoin('oc_yzc_rma_order_product as rop','ro.id','=','rop.rma_id')
            ->where('ro.rma_order_id','=',$rma_order_id)
            ->select('ro.id','ro.order_id','rop.product_id','rop.quantity','rop.apply_refund_amount','rop.reason_id','rop.comments','rop.order_product_id')
            ->first();
	    return $result;
    }
