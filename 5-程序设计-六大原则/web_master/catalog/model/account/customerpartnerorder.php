...
/**
 * Class ModelAccountCustomerpartnerOrder
 ...
 * @property ModelExtensionShippingwkcustomshipping $model_extension_shipping_wk_custom_shipping
 */
...
//					$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_product SET quantity = quantity-'".(int)$product['quantity']."' WHERE product_id = '".(int)$mpSellers['product_id']."' AND customer_id = '".(int)$mpSellers['customer_id']."'");
	            ...
	            if ($this->config->get('marketplace_low_stock_notification') && (int)$this->config->get('marketplace_low_stock_quantity') && isset($product_current_quantity['quantity']) && $product_current_quantity['quantity'] <= (int)$this->config->get('marketplace_low_stock_quantity')) {
	...
	public function getSellerOrderInfo($order_id, $customer_id){
        /**
         * 去除ACME购买流程校验
         */
		$sql = "SELECT * FROM " . DB_PREFIX . "customerpartner_to_order cto WHERE cto.order_id = " . $order_id . " AND cto.customer_id not in (1,2,3,14,15) ";
		return $this->db->query($sql);
	}

    public function getSellerOrderInfoByOrderId($order_id){
        $sql = "SELECT * FROM " . DB_PREFIX . "customerpartner_to_order cto WHERE cto.order_id = " . $order_id;
        return $this->db->query($sql);
    }
