<?php
/**
 * Class ModelAccountRmawkrmaadmin
 * Webkul Software.
 * @category  Webkul
 * @author    Webkul
 ...
 */
class ModelAccountRmawkrmaadmin extends Model {

	use RmaModelTrait;
	use RmaModelRmaTrait;

	public function verifyReasonForSeller($id, $seller_id){
    $sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "wk_rma_reason` WHERE reason_id='".(int)$id."'");
		if ($sql->num_rows == 1 && $sql->row['seller_id'] == $seller_id) {
			return true;
		}
    return false;
  }

	public function verifyStatusForSeller($id, $seller_id){
	...
	public function verifyRmaForSeller($id, $seller_id){
		$sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "wk_rma_order` WHERE id='".(int)$id."'");
		if ($sql->num_rows == 1 && $sql->row['seller_id'] == $seller_id) {
			return true;
		}
		return false;
	...
	public function updateAdminStatus($msg,$status,$vid,$file_name){
		...
		if($msg!=''){
			$this->db->query("INSERT INTO " . DB_PREFIX . "wk_rma_order_message SET rma_id = '".$this->db->escape($vid)."', writer = 'seller', message = '".$this->db->escape(nl2br($msg))."', date = NOW(), attachment = '".$this->db->escape($file_name)."'");
		}
		...
		$this->mail($data,'message_to_seller_on_admin_message');
