...
class Loader
{
    ...
    public function controller($route, $data = array())
    {
        ...
        if ($result != null && !$result instanceof Exception) {
            ...
            $action = new \Framework\Action\Action($route);
    ...
    /**
     * 如果一个Model尚未注册，则 include 其类文件，并注册到 registry 的data中。
     *
     * @param string $route
     * @throws Exception
     */
     ...
     * @return    string
     */
    public function view($route, $data = array(), $layout = '')
    {
        ...
        if ($result && !$result instanceof Exception) {
            ...
            $data[TwigRenderer::TWIG_DATA_NOT_EXIST_CHECK_KEY] = array_keys($dataFromController); // 仅从控制器过来的参数需要检查是否被使用
            $output = view($route, $data, $layout);
        }
