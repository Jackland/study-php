...
class Squareup {
    ...
    public function authLink($client_id) {
        ...
        $redirect_uri = str_replace('&amp;', '&', $this->url->link('extension/payment/squareup/oauth_callback', 'user_token=' . session('user_token'), true));

        session()->set('payment_squareup_oauth_redirect', $redirect_uri);
    ...
    protected function authState() {
        if (!isset($this->session->data['payment_squareup_oauth_state'])) {
            session()->set('payment_squareup_oauth_state', bin2hex(openssl_random_pseudo_bytes(32)));
        }
