	...
	public function send() {
		...
		foreach ($this->attachments as $attachment) {
			...
			if (file_exists($attachment['path'])) {
				...
				$content = fread($handle, filesize($attachment['path']));

				fclose($handle);

                $file_name = is_null($attachment['name'])? basename($attachment['path']) : $attachment['name'];
//                $gb_name = mb_convert_encoding($file_name, "GBK");
				$message .= '--' . $boundary . PHP_EOL;
				$message .= 'Content-Type: application/octet-stream; name="' . $file_name . '"' . PHP_EOL;
				$message .= 'Content-Transfer-Encoding: base64' . PHP_EOL;
//				$message .= 'Content-Disposition: attachment; filename*=GBK\'\'' . $gb_name . '' . PHP_EOL;
				$message .= 'Content-Disposition: attachment; filename="=?UTF-8?B?' . base64_encode($file_name) . '?="' . PHP_EOL;
				$message .= 'Content-ID: <' . urlencode($file_name) . '>' . PHP_EOL;
				$message .= 'X-Attachment-Id: ' . urlencode($file_name) . PHP_EOL . PHP_EOL;
...
//			fputs($handle, 'EHLO ' . getenv('SERVER_NAME') . "\r\n");
			fputs($handle, 'EHLO ' . 'localhost:84' . "\r\n");
