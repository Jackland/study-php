...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
        ...
        if ($this->config->get('module_marketplace_status') && isset($this->request->get['mpcheck'])) {
            $this->response->redirect($this->url->link('customerpartner/product', 'user_token=' . session('user_token') . $url, true));
        } else {
            $this->response->redirect($this->url->link('catalog/product', 'user_token=' . session('user_token') . $url, true));
        }
         /**
         * Marketplace Code Ends Here
         */
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/product', 'user_token=' . session('user_token') . $url, true)
		);

		$data['add'] = $this->url->link('catalog/product/add', 'user_token=' . session('user_token') . $url, true);
		$data['copy'] = $this->url->link('catalog/product/copy', 'user_token=' . session('user_token') . $url, true);
		$data['delete'] = $this->url->link('catalog/product/delete', 'user_token=' . session('user_token') . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'edit'       => $this->url->link('catalog/product/edit', 'user_token=' . session('user_token') . '&product_id=' . $result['product_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . '&sort=pd.name' . $url, true);
		$data['sort_model'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . '&sort=p.model' . $url, true);
		$data['sort_price'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . '&sort=p.price' . $url, true);
		$data['sort_quantity'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . '&sort=p.quantity' . $url, true);
		$data['sort_status'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . '&sort=p.status' . $url, true);
		$data['sort_order'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . '&sort=p.sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/product', 'user_token=' . session('user_token') . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['product_id'])) {
			$data['action'] = $this->url->link('catalog/product/add', 'user_token=' . session('user_token') . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/product/edit', 'user_token=' . session('user_token') . '&product_id=' . $this->request->get['product_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/product', 'user_token=' . session('user_token') . $url, true);
        ...
        if ($this->config->get('module_marketplace_status') && isset($this->request->get['mpcheck'])) {
            if (!isset($this->request->get['product_id'])) {
                $data['action'] = $this->url->link('catalog/product/add', 'user_token=' . session('user_token') . $url . '&mpcheck=1', true);
            } else {
                $data['action'] = $this->url->link('catalog/product/edit', 'user_token=' . session('user_token') . '&mpcheck=1&product_id=' . $this->request->get['product_id'] . $url, true);
            }

            $data['cancel'] = $this->url->link('customerpartner/product', 'user_token=' . session('user_token') . $url, true);
        } else {
            if (!isset($this->request->get['product_id'])) {
                $data['action'] = $this->url->link('catalog/product/add', 'user_token=' . session('user_token') . $url, true);
            } else {
