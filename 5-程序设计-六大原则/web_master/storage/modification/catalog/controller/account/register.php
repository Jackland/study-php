...
//			$customer_id = $this->model_account_customer->addCustomer($this->request->post);
        ...
        if (isset($this->error['cardNumber'])) {
            $data['error_cardNumber'] = $this->error['cardNumber'];
        } else {
            $data['error_cardNumber'] = '';
        }
        if (isset($this->error['cardName'])) {
            $data['error_cardName'] = $this->error['cardName'];
        } else {
            $data['error_cardName'] = '';
        }
        if (isset($this->error['billingAddress'])) {
            $data['error_billingAddress'] = $this->error['billingAddress'];
        } else {
            $data['error_billingAddress'] = '';
        }

        if (isset($this->error['invitationCode'])) {
            $data['error_invitationCode'] = $this->error['invitationCode'];
        } else {
            $data['error_invitationCode'] = '';
        }
        //add end
	...
	private function validate() {
        ...
        //校验 cardNumber
        if ($this->request->post['cardNumber']!='' and preg_match('/^[0-9]+$/',$this->request->post['cardNumber']) != 1) {
            $this->error['cardNumber'] = $this->language->get('error_cardNumber');
        }
        //校验 cardName
        if ($this->request->post['cardName']!='' and preg_match('/^[A-Za-z]+$/',$this->request->post['cardName']) != 1) {
            $this->error['cardName'] = $this->language->get('error_cardName');
        }
        //校验 street address
        if ($this->request->post['street']!='' and preg_match('/^[A-Za-z0-9\s]+$/',$this->request->post['street']) != 1) {
            $this->error['street'] = $this->language->get('error_street');
        }
        //校验 billingAddress
        if ($this->request->post['billingAddress']!='' and preg_match('/^[A-Za-z,]+$/',$this->request->post['billingAddress']) != 1) {
            $this->error['billingAddress'] = $this->language->get('error_billingAddress');
        }
        //邀请码校验
        if ($this->request->post['invitationCode']=='' || !$this->model_account_customer->getInvitation($this->request->post['invitationCode'])) {
            $this->error['invitationCode'] = $this->language->get('error_invitationCode');
        }
		// Agree to terms
		if ($this->config->get('config_account_id')) {
			$this->load->model('catalog/information');
    ...
    public function invitation(){
        ...
        $invivation = $this->model_account_customer->getInvitation($this->request->get['invitation_code']);
        if ($invivation) {
            $json = array(
                'status' => true
            );
